...
static int smap(HMM *, char);
static int tmap(HMM *, char);
static void alphaT(HMM *, double *, char *);
static void beta1(HMM *, double *, char *);
static double * xi(HMM *);
static void alpha_mat(HMM *, double **, char *);
static void beta_mat(HMM *, double **, char *);
...
{
   return hmm->init[tmap(hmm, state[0])];
}
...
{
   return hmm->a_mat[tmap(hmm, state1[0])][tmap(hmm, state2[0])];
}
...
{
   hmm->a_mat[tmap(hmm, state1[0])][tmap(hmm, state2[0])] = val;
}
...
{
   return hmm->e_mat[tmap(hmm, state[0])][smap(hmm, symbol[0])];
}
...
{
   hmm->e_mat[tmap(hmm, state[0])][smap(hmm, symbol[0])] = val;
}

/*
   smap takes a symbol character and return its index in the state
   transition matrix.
 */
static int
smap(HMM * hmm, char c)
...
/*
   tmap takes a state character and return its index in the emission
   matrix.
 */
static int
tmap(HMM * hmm, char c)
 ...
 */
static void
alphaT(HMM * hmm, double * alpha_vec, char * obs)
{
   int N = hmm->N;
   ...
   for (i  = 0; i < N; ++i)  
      alpha_vec[i] = log(init[i]) + log(e_mat[i][smap(hmm, obs[0])]);
   if (T == 1) 
      return;
   if (T > 1) {
      for (i = 1; i < T; ++i) {
         ...
         for (j = 0; j < N; ++j) {
            ...
            alpha_vec[j] = alpha_vec[j] + log(e_mat[j][smap(hmm, obs[i])]);
         }
      }
    }
    return; 
 ...
 */
static void
beta1(HMM * hmm, double * beta_vec, char * obs)
{
   int N = hmm->N;
   ...
   if (T > 1) {
      for (i = T - 1; i >= 1; --i) {
         ...
         for (j = 0; j < N; ++j) {
            for (k = 0; k < N; ++k)
               beta_vec[j] = sumLogProb(beta_vec[j], beta_vec_next[k] + log(a_mat[j][k]) + log(e_mat[k][smap(hmm, obs[i])]));
         }
      }
    }
    return; 
...
Pbeta(HMM * hmm, char * obs)
{
   ...
   beta1(hmm, beta_vec, obs);
   for (i = 0; i < hmm->N; ++i) {
      P = sumLogProb(P, log(hmm->init[i]) + log(hmm->e_mat[i][smap(hmm, obs[0])]) + beta_vec[i]);
   }
   free(beta_vec);
   return P;
}
 ...
 */  
static void
alpha_mat(HMM * hmm, double ** alpha, char * obs)
{
   int i, j, t;
   int N = hmm->N, T = strlen(obs);
   double ** a_mat = hmm->a_mat;
   double ** e_mat = hmm->e_mat;
   double * init = hmm->init;

   for (i = 0; i < N; ++i) 
      alpha[i][0] = log(init[i]) + log(e_mat[i][smap(hmm, obs[0])]);
   for (t = 1; t < T; ++t) {
      for (i = 0; i < N; ++i) 
         alpha[i][t] = log(0);
      for (i = 0; i < N; ++i) {
         ...
         alpha[i][t] += log(e_mat[i][smap(hmm, obs[t])]);
      }
   }
}
 ...
 */  
static void
beta_mat(HMM * hmm, double ** beta, char * obs)
{
   int i, j, t;
   int N = hmm->N, T = strlen(obs);
   double ** a_mat = hmm->a_mat;
   ...
   for (t = T-2; t >= 0; --t) {
      ...
      for (i = 0; i < N; ++i) 
         for (j = 0; j < N; ++j) 
            beta[i][t] = sumLogProb(beta[i][t], beta[j][t+1] + log(a_mat[i][j]) + log(e_mat[j][smap(hmm, obs[t+1])]));
   }
}
 ...
 */
void
baum_welch(HMM * hmm, char ** obs, int L)
{
   int i, j, l, t;
   ...
   for (l = 0; l < L; ++l) {
      P[l] = Palpha(hmm, obs[l]);
      if (maxL == -1) 
         maxL = strlen(obs[l]);
      else if (strlen(obs[l]) > maxL)
         maxL = strlen(obs[l]);
   }
...
/* use all training sequences to train */
      for (l = 0; l < L; ++l) {
         T = strlen(obs[l]);
         alpha_mat(hmm, alpha, obs[l]);
         beta_mat(hmm, beta, obs[l]);

/* estimate new initial state probability */
         for (i = 0; i < N; ++i) 
            for (j = 0; j < N; ++j) 
               init[i] += exp(alpha[i][0] + log(hmm->a_mat[i][j]) + log(hmm->e_mat[j][smap(hmm, obs[l][1])]) + beta[j][1] - P[l]);

/* estimate A matrix */
         for (i = 0; i < N; ++i) 
            for (j = 0; j < N; ++j) 
               for (t = 0; t < T-1; ++t) 
                  A[i][j] += exp(alpha[i][t] + log(hmm->a_mat[i][j]) + log(hmm->e_mat[j][smap(hmm, obs[l][t+1])]) + beta[j][t+1] - P[l]);

/* Estimate E matrix */
         for (i = 0; i < N; ++i)
            for (t = 0; t < T; ++t)
               E[i][smap(hmm, obs[l][t])] += exp(alpha[i][t] + beta[i][t] - P[l]);
      }
 ...
 */
void
viterbi(HMM * hmm, char * hss, char * obs)
{
   ...
   if (T > 1) {
      for (i = 0; i < T; ++i) {
         ...
         for (j = 0; j < N; ++j) {
            ...
            delta_vec[j] = max + log(e_mat[j][smap(hmm, obs[i])]);
            phi_mat[i][j] = max_i;
         }
      }
   }
 ...
 */
void
state_est(HMM * hmm, char ** obs, char ** sta, int L)
{
   int N = hmm->N, M = hmm->M;
   int T;
   int i, j, k, l;
...
/* count occurrences */
   for (l = 0; l < L; ++l) {
      ...
      for (i = 0; i < T - 1; ++i)
         ++A[tmap(hmm, sta[l][i])][tmap(hmm, sta[l][i+1])];
      for (i = 0; i < T; ++i)
         ++E[tmap(hmm, sta[l][i])][smap(hmm, obs[l][i])];
   }
